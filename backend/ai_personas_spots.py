"""
AI Surf Analysis Personas for SURF SPOTS (not buoys).
...
async def get_spot_context(spot_slug: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        # Admin client bypasses RLS so private spots (visibility='private') are
        # readable too. The caller is already auth-gated; here we just need the
        # spot data, not a re-check.
        try:
...
class SpotSwellGeometryAnalyst:
    """
    ...
    async def analyze(spot_slug: str) -> Dict[str, Any]:
        """
        Analyze surf spot swell geometry using database context.
        """
...
async def analyze_spot_swell_geometry(spot_slug: str) -> Dict[str, Any]:
    """
    Main entry point for spot swell geometry analysis.
    """
    return await SpotSwellGeometryAnalyst.analyze(spot_slug)
