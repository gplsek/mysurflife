...
### 3.6 Quick chip — "Ask Scout about this storm" (`.l1-quick-chip`)
...
- `--aqua` color, 12px weight 500, chat-bubble SVG + label `"Ask Scout about this storm"` + `.spark` trailing text `"Chat ↗"` (Geist Mono 10px muted)

**Click behavior (production):** Opens a new Scout session scoped to this storm context but **no specific spot**. The opening assistant message should summarize the storm and ask a general question like "Want me to break down which coasts this hits?" — contrast with per-spot "Plan trip" (§4.5) which is spot-scoped.
...
| Button | Class | Action |
|---|---|---|
| `Plan trip with Scout` | `.spot-cta.primary` | Hands off to Scout assistant with full context (storm + region + spot + arrivals + user home location). See §8. |
| `Open spot` | `.spot-cta` | Navigate to `/spots/{spot.id}` (prototype: static `mysurflife-spot-detail.html`) |
| `Set alert` | `.spot-cta` | Open Alerts form pre-filled with `{storm_id, spot_id, window: first→first+window_h}`. Direct form, not via Scout. |
...
## 8. "Plan trip with Scout" handoff (§ spec)

When the L3 primary CTA is clicked:

1. POST `/api/scout/sessions` with:
   ```json
   {
     "context": {
       "source": "storm-card",
...
3. Navigate to `/copilot?session={session_id}` (or in-prod final Scout route).
...
| 10 | "Ask Scout" chip | Button present, no route | Opens a Scout session scoped to the storm but not a spot. |
| 11 | "Plan trip with Scout" | Button present, no route | Full handoff per §8. |
...
- No inline chat UI — Scout handoff only. Conversations live in `/copilot`.
